...
public class YeWorkerController
{
	...
	public String toIndex(){
		return "yeloginfornow";
	}

	@RequestMapping("/yelogin.do")
	public String loginToMian(String account, String password, HttpServletRequest request){
		YeWorker worker =yeWorkerService.workerLogin(account);
		String MD5Pass= Md5utils.getMD5LowerCase(password);
		if(worker!=null&&worker.getWpass().equals(MD5Pass)){
			...
			YeWorker yeWorker = yeWorkerMapper.selectWorker(workAccount);
			Worker worker1 = new Worker();
			worker1.setWid(yeWorker.getWid());
	...
	public String entityRegister(HttpSession session,String wname, String waccount, String wpass, String wphone, String wemail ){
		...
		String md5Pass = Md5utils.getMD5LowerCase(wpass);
		boolean register = yeWorkerService.groupClientRegister(waccount, wname, md5Pass, wphone, wemail);
	...
	public String singleBillForm(HttpServletRequest request,String waccount,String wpass,String sphone, String sname,String sex,String spackage,String sid){
		...
		String md5Pass = Md5utils.getMD5LowerCase(wpass);
		if(yeGroupClient!=null){
			if(yeGroupClient.getGcpass().equals(md5Pass)){
	...
	public String repeatOrder(HttpServletRequest request,HttpServletResponse response,String gcaccount,String gcpass){
		YeGroupClient yeGroupClient = yeWorkerMapper.selectGcAccount(Long.valueOf(gcaccount));
		String md5Pass = Md5utils.getMD5LowerCase(gcpass);
		if(!yeGroupClient.getGcpass().equals(md5Pass)){
