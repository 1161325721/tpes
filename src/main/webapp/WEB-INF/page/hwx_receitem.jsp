...
<%--		根据项目的类型进行选择 sum_type--%>
		...
		<div style="float: left;width: 300px;height: 300px;" >
			...
			<textarea class="layui-textarea" name="sumtext" id="sumtext1" cols="40" rows="11"  placeholder="小结内容" readonly style="width: 300px;height: 300px;"></textarea>
		</div>
...
<div id="showlist" style="display: none;width:500px;height:800px ">
	...
	<div>
		<textarea class="layui-textarea" name="sumtext2" id="sumtext2" cols="30" rows="10" placeholder="小结内容" readonly style="width: 600px;height: 200px;"> </textarea>
	</div>
...
<%--上传影像图片、影像小结--%>
<div id="uploadpic" style="display: none;width:600px;height:800px " >
	<div style="width: 600px;">
		<label id="lbupload" class="layui-form-label" style="background-color: #95877c;width:560px;text-align: left">
			影像上传
		</label>
	</div>
	<br>
<%--	上传按钮--%>
	<div class="layui-upload" style="float: left;width: 350px;">
<%--		上传按钮--%>
	<button class="layui-btn" id="uploadbtn" lay-event="uploadbtn" style="margin-left:35%;margin-top:25%;"><i class="layui-icon"></i>上传图片</button>
	...
	<div style="width: 600px;">
		<label id="writeSum" name = "writeSum" class="layui-form-label" style="background-color: #95877c;width:560px;text-align: left">
			编写小结
		</label>
	</div>
	<div>
...
<%--		编辑 监听按钮状态 先询问是否修改 确认在发送ajax--%>
		<button class="layui-btn" id="editsum1" lay-event="editsum1" style="margin-left: 40%;">编辑</button>
<%--		提交 监听按钮状态 先验证（为空、提示最后一次无法在修改）在提交ajax --%>
		<button class="layui-btn" id="submit1" lay-event="submit1" style="margin-left: 10%;">提交</button>
	</div>
</div>
...
<script type="text/html" id="bar" lay-filter ="btnaction">
	{{#  if(d.cpstate ==1&&d.gostate==1&&d.sumState==0 ){   }}
	...
	layui.use(['form','layer','jquery','table'], function(){
		...
		table.render({
			...
			,cols: [[
				...
				,{field:'sumType', width:100, title: '项目类型',templet :function(d){
							return d.sumType== 'img' ? '影像' : '普通';
						}}
				...
				,{field:'sumState', width:100, title: '是否小结',templet :function(d){
						return d.sumState == 1 ? '是' : '否';
					}}
			...
			switch(obj.event){
				...
				case "findsum":
					...
					// layer.msg("data" + data);
					 ...
					 // ajax发送数据刷新列表
					// 设置图片
					// layer.msg("img :" + data.pic);

					$('#sumimg').attr('src',data.pic); //图片链接（base64）
